...
final class TestBase58 extends AnyFunSuite with Matchers {
  ...
  private[this] val data: Vector[(Array[Byte],String)] = Vector(
    ...
    (Base16.decode("00010966776006953D5567439E5E39F86A0D273BEED61967F6".toArray), "16UwLL9Risc3QfPqBUvKofHmBQ7wMtjvM")
  )
...
//  }
  ...
  private def check(bytes: Array[Byte], encoded: String): Unit = {
    ...
    Base58.decode(encoded.getBytes(StandardCharsets.UTF_8)) shouldBe bytes

    // def decode(data: ByteBuffer)
    Base58.decode(ByteBuffer.wrap(encoded.getBytes(StandardCharsets.UTF_8))).array() shouldBe bytes
