 ...
 */
protected trait ImplicitsBase extends OrderingImplicits with AnyRefNullCheckOps with TypeSafeEqualsOps with PlatformCompat with ImplicitsCollectionCompat {
...
//  implicit def toAnyRefNullChecks[A <: AnyRef](ref: A): AnyRefNullChecks[A] = new AnyRefNullChecks[A](ref)
  ...
  implicit def toRichGrowable[A](growable: GrowableCompat.Growable[A]): RichGrowable[A] = new RichGrowable(growable)
  implicit def toRichTraversableOnce[A, COL](col: COL)(implicit ev: COL => TraversableOnce[A]): RichTraversableOnce[A, COL] = new RichTraversableOnce(col)
  implicit def toRichIterable[A](col: Iterable[A]): RichIterable[A] = new RichIterable(col)
  implicit def toRichIndexedSeq[T](t: scala.collection.IndexedSeq[T]): RichIndexedSeq[T] = new RichIndexedSeq(t)
...
//  implicit def toTypeSafeEquals[L](left: L): TypeSafeEquals[L] = new TypeSafeEquals(left)
  ...
  implicit def toRichJavaDuration(duration: java.time.Duration): RichJavaDuration = new RichJavaDuration(duration)
  implicit def toRichScalaDuration(duration: scala.concurrent.duration.FiniteDuration): RichScalaDuration = new RichScalaDuration(duration)
}