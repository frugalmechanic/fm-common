...
final class RichLocalDate(val date: LocalDate) extends AnyVal with Ordered[LocalDate] {
  def compare(that: LocalDate): Int = LocalDateCompat.compare(date, that)
  ...
  def toYYYYMMDD: String = {
    ...
    val year: Int = LocalDateCompat.getYear(date)
    val month: Int = LocalDateCompat.getMonthValue(date)
    val day: Int = LocalDateCompat.getDayOfMonth(date)
  ...
  def toYYYYMMDDInt: Int = {
    yyyyRangeCheck()
    LocalDateCompat.getYear(date) * 10000 + LocalDateCompat.getMonthValue(date) * 100 + LocalDateCompat.getDayOfMonth(date)
  }
