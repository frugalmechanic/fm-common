...
//  2021-09-28 - We don't have easy access to SHA256 hashing in Scala.js and we don't use checked encoding so disable this
//  /**
...
//  }
  ...
  override def decode(input: Array[Byte]): Array[Byte] = {
    decode(ByteBuffer.wrap(input)).array()
  }

  override def decode(input: ByteBuffer): ByteBuffer = {
    if (input.remaining() == 0) return ByteBuffer.wrap(new Array[Byte](0))

    val length: Int = input.remaining()
    val offset: Int = input.position()

    // Convert the base58-encoded ASCII chars to a base58 byte sequence (base58 digits).
    val input58: Array[Byte] = new Array(length)

    var i: Int = 0

    while (i < length) {
      val c: Byte = input.get(offset + i)
      val digit: Int = if (c < 128) INDEXES(c) else -1
      if (digit < 0) throw new IllegalArgumentException("Invalid Character: "+c.toChar+" at idx: "+i)
    ...
    val decoded: Array[Byte] = new Array(length)
    ...
    ByteBuffer.wrap(Arrays.copyOfRange(decoded, outputStart - zeros, decoded.length))
  }

  override def decode(input: Array[Char]): Array[Byte] = {
   ...
   */
  override def decode(input: CharSequence): Array[Byte] = {
...
//  2021-09-28 - We don't have easy access to SHA256 hashing in Scala.js and we don't used checked encoding so disable this
//  /**
