...
abstract class ReloadableFileResource[T] extends ReloadableResource[T] {
  ...
  private def sortedFilesToTry: Seq[File] = {
    resourceFiles.filter{ (f: File) => f.isFile && f.canRead }.sortBy{ _.lastModified }.reverse
  }

  protected def loadFromPrimary(): Option[T] = {
    sortedFilesToTry.foreach{ (file: File) =>
      try {
  ...
  protected def loadFromBackup(): Option[T] = backupResourcePath.map{ (path: String) =>
