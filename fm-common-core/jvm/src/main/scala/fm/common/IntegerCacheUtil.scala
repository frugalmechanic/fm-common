...
protected[common] object IntegerCacheUtil {
  val low: Int = try {
    // Note: Do not rely on .toIntOptionCached here since this is used to initialize the Option cache
    System.getProperty("fm.common.IntegerCache.low").toInt
  } catch {
    case _: NumberFormatException => -128
  }
  ...
  val high: Int = try {
    // Note: Do not rely on .toIntOptionCached here since this is used to initialize the Option cache
    System.getProperty("fm.common.IntegerCache.high").toInt
  } catch {
    case _: NumberFormatException => determineIntegerCacheHighValue()
  }
