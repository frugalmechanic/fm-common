...
final class RichIterableOnce[A](val self: IterableOnce[A]) extends AnyVal {

  def minOption[B >: A](implicit cmp: Ordering[B]): Option[A] = {
    if (self.iterator.isEmpty) None
    else Some(self.iterator.min[B])
  }
  
  def maxOption[B >: A](implicit cmp: Ordering[B]): Option[A] = {
    if (self.iterator.isEmpty) None
    else Some(self.iterator.max[B])
  }
  
  /**
   * Same as mkString except if the IterableOnce is empty then an empty
  ...
  def sortByCached[K](f: A => K)(implicit ord: Ordering[K]): IndexedSeq[A] = {
    val orig: IndexedSeq[A] = self.iterator.toIndexedSeq
  ...
  def findMappedFuture[B](f: A => Future[Option[B]])(implicit ec: ExecutionContext): Future[Option[B]] = {
    self.iterator.foldLeft[Future[Option[B]]](Future.successful(None)){ (prev: Future[Option[B]], next: A) =>
...
//      val key: K = x._1
//            
//      if (m.contains(key) && (strict || m(key) != x._2)) throw new IllegalArgumentException(s"RichIterableOnce.toUniqueHashMap - Map already contains key: $key   Existing Value: ${m(key)}  Trying to add value: ${x._2}")
//      
//      m += x
//    }
//    
